...
class CustomContentTest(TestCase):
    def test_get_absolute_url(self):
        cj = mommy.make('djthirdparty.CustomContent', path='/hello/')

        self.assertEqual(cj.get_absolute_url(), '/hello/')

    def test_unicode(self):
        cj = mommy.make(
            'djthirdparty.CustomContent', path='/hello/', title='world')

        self.assertEqual(unicode(cj), 'world - /hello/')

    def test_clean_both_exact_and_partial_match(self):
        cj = mommy.make(
            'djthirdparty.CustomContent', exact_match=True, partial_match=True)
    ...
    def test_clean_valid_regex(self):
        cj = mommy.make(
            'djthirdparty.CustomContent', exact_match=False,
